 ...
 * Uses MD5 hash as required by Gravatar's API.
 * Falls back to a "retro" default avatar if no Gravatar exists for this email.
 */
...
function md5(input: string): string {
  ...
  function binlMD5(x: number[], len: number) {
    ...
    for (let i = 0; i < x.length; i += 16) {
      ...
      a = md5gg(a, b, c, d, x[i + 13], 5, -1444681467); d = md5gg(d, a, b, c, x[i + 14 - 1], 9, -51403784)
      c = md5gg(c, d, a, b, x[i + 2], 14, 1735328473); b = md5gg(b, c, d, a, x[i + 7], 20, -1926607734)
...
/**
 * Smart profile picture URL with fallback chain:
 * 1. Stored profilePictureUrl (from MS Graph, uploaded, etc.)
 * 2. Gravatar based on email
 * 
 * @param options - User profile options
 * @returns Profile picture URL
 */
export function getProfilePictureUrl(options: {
  profilePictureUrl?: string | null
  email?: string | null
  displayName?: string | null
  size?: number
}): string {
  const { profilePictureUrl, email, size = 80 } = options
  
  // Priority 1: Use stored profile picture URL (e.g., from Microsoft Graph)
  if (profilePictureUrl && profilePictureUrl.trim()) {
    return profilePictureUrl
  }
  
  // Priority 2: Use Gravatar based on email
  if (email && email.trim()) {
    return getGravatarUrl(email, size)
  }
  
  // Fallback: Return a default retro avatar with empty hash
  return `https://www.gravatar.com/avatar/00000000000000000000000000000000?s=${size}&d=retro`
}

/**
 * Check if a URL is a valid image (for profile pictures)
